...
describe( 'Capture Constraints', () => {
    ...
    it( 'Should let us construct instances and query their members', () => {
        ...
        b = new LurchSymbol( 'bound' )
        f = new LurchSymbol( 'free' )
    ...
    it( 'Should let us make copies and compare for equality', () => {
        ...
        let notCopy1 = new M.CaptureConstraint( new LurchSymbol( 'x' ), f )
        let notCopy2 = new M.CaptureConstraint( b, new LurchSymbol( 'y' ) )
        let notCopy3 = new M.CaptureConstraint(
    ...
    it( 'Should know when an instance is complete', () => {
        ...
        CC = new M.CaptureConstraint(
            new LurchSymbol( 'not a metavariable' ),
            new LurchSymbol( 'also not a metavariable' )
        )
        expect( CC.complete() ).to.equal( true )
        CC = new M.CaptureConstraint(
            new LurchSymbol( 'ooh, a metavariable' ).asA( M.metavariable ),
            new LurchSymbol( 'also not a metavariable' )
        )
        expect( CC.complete() ).to.equal( false )
        CC = new M.CaptureConstraint(
            new LurchSymbol( 'not a metavariable' ),
            new LurchSymbol( 'this one is a metavariable' ).asA( M.metavariable )
        )
        expect( CC.complete() ).to.equal( false )
        CC = new M.CaptureConstraint(
            new LurchSymbol( 'ooh, a metavariable' ).asA( M.metavariable ),
    ...
    it( 'Should know when an instance is satisfied/violated', () => {
        ...
        f = new LurchSymbol( 'temp' )
        CC = new M.CaptureConstraint( b, f )
        CC.free = new Application( new LurchSymbol( 'some' ), b.copy() )
    ...
    it( 'Should let us apply Constraints to it', () => {
        ...
        // Case 1:
        b = new LurchSymbol( 'thing' ).asA( M.metavariable )
        f = new LurchSymbol( 'temp' ).asA( M.metavariable )
        CC = new M.CaptureConstraint( b, f )
        S = new Substitution( b, LogicConcept.fromPutdown( '"ho ho ho"' )[0] )
        // try appliedTo() in this case
        expect( () => result = S.appliedTo( CC ) ).not.to.throw()
        // ensure that CC has not changed
        expect( CC.bound ).to.equal( b )
        expect( CC.free ).to.equal( f )
        // ensure that the result had S applied to it
        expect( result.bound.equals( S.expression ) ).to.equal( true )
        expect( result.bound ).not.to.equal( S.expression )
        expect( result.free.equals( f ) ).to.equal( true )
        // try applyTo() in this case
        expect( () => S.applyTo( CC ) ).not.to.throw()
        // ensure that CC has changed
        expect( CC.bound.equals( S.expression ) ).to.equal( true )
        expect( CC.bound ).not.to.equal( S.expression )
        ...
        f = new LurchSymbol( 'temp' ).asA( M.metavariable )
        CC = new M.CaptureConstraint( b, f )
        S = new Substitution( f, LogicConcept.fromPutdown( '((1 2) 3)' )[0] )
        // try appliedTo() in this case
        expect( () => result = S.appliedTo( CC ) ).not.to.throw()
        // ensure that CC has not changed
        expect( CC.bound ).to.equal( b )
        ...
        expect( result.free.equals( S.expression ) ).to.equal( true )
        expect( result.free ).not.to.equal( S.expression )
        // try applyTo() in this case
        expect( () => S.applyTo( CC ) ).not.to.throw()
        // ensure that CC has changed
...
describe( 'Capture Constraint sets', () => {
    ...
    it( 'Should correctly construct instances from capture constraints', () => {
        ...
        ccon1 = new M.CaptureConstraint(
            new LurchSymbol( 'x' ), new LurchSymbol( 'y' ) )
        ccon2 = new M.CaptureConstraint(
            new LurchSymbol( 'a' ), new LurchSymbol( 'y' ) )
        ccon3 = new M.CaptureConstraint(
            new LurchSymbol( 'x' ), new LurchSymbol( 'y' ) )
    ...
    it( 'Should correctly simplify instances', () => {
        ...
        C = new M.CaptureConstraints(
            new M.CaptureConstraint(
                new LurchSymbol( 1 ), new LurchSymbol( 2 ) ),
            new M.CaptureConstraint(
                new LurchSymbol( 3 ).asA( M.metavariable ),
                new LurchSymbol( 4 ) )
        )
        expect( C.constraints.length ).to.equal( 2 )
        expect( () => C.simplify() ).not.to.throw()
        ...
        C.constraints[0].bound = new LurchSymbol( 'and we are good' )
        expect( C.constraints.length ).to.equal( 1 )
        expect( () => C.simplify() ).not.to.throw()
        expect( C.constraints.length ).to.equal( 0 )
        expect( C.violated() ).to.equal( false )
